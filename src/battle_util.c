...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case ABILITY_STEELY_SPIRIT:
        if (moveType == TYPE_STEEL)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_ILLUSION: // kleen custom
...
void CopyMonAbilityAndTypesToBattleMon(u32 battler)
{
    FillBattlerTypesFromParty(battler);
    gBattleMons[battler].ability = GetMonDataFromBattler(battler, MON_DATA_ABILITY);
}

void FillBattlerTypesFromParty(u32 battler)
{
    gBattleMons[battler].type1 = GetMonDataFromBattler(battler, MON_DATA_TYPE1);
    gBattleMons[battler].type2 = GetMonDataFromBattler(battler, MON_DATA_TYPE2);  
    gBattleMons[battler].type3 = TYPE_MYSTERY;  
}

u32 GetMonDataFromBattler(u32 battlerId, u32 data)
{
    // careful not requesting something like Nickname or something
    return GetMonData(GetBattlerPartyData(battlerId), data, NULL);
}

struct Pokemon *GetBattlerPartyData(u32 battlerId)
{
    struct Pokemon *mon;
    if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
...
void RecalcBattlerStats(u32 battler, struct Pokemon *mon)
{
    ...
    CopyMonAbilityAndTypesToBattleMon(battler);
}
