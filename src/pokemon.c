...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    case MON_DATA_CUSTOM_ABILITY:
        if (boxMon->abilityNum < 4) // ability nnum is used normally
            retVal = 0;
        else // ability stores the index of the custom ability minus 3.
            retVal = boxMon->abilityNum -3; // 
        break;
...
u16 GetDataCalculatedStats(struct Pokemon *mon, u8 ivid, u8 evid, u8 customstatid, u8 statIndex)
{
    ...
    u16 species = GetMonData(mon, MON_DATA_SPECIES, NULL);
    u16 baseStat =  GetSpeciesBaseStat(species, statIndex) + (10 * GetMonData(mon, customstatid, NULL));
