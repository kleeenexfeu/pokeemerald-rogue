...
#define SERIALIZED_DATA_SIZE(member) sizeof(member)
#define SERIALIZED_ARRAY_SIZE(member) (sizeof(u16) + sizeof(member))

#ifdef ROGUE_DEBUG
#define ROGUE_SERIALIZED_DEBUG_CONFIG_SIZE \
    (SERIALIZED_ARRAY_SIZE(gRogueDebug.toggleBits) + SERIALIZED_ARRAY_SIZE(gRogueDebug.rangeValues))
#else
#define ROGUE_SERIALIZED_DEBUG_CONFIG_SIZE 0
#endif

#define ROGUE_SAVE_SERIALIZED_SIZE(saveBlock) ( \
    SERIALIZED_DATA_SIZE((saveBlock)->saveVersion) + \
    SERIALIZED_DATA_SIZE((saveBlock)->currentBlockFormat) + \
    sizeof(u16) + /* secretId */ \
    sizeof(u8) + /* rogueVersion */ \
    SERIALIZED_ARRAY_SIZE((saveBlock)->questStates) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->campaignData) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.areaBuiltFlags) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.areaCoords) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.upgradeFlags) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.homeDecorations) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.homeStyles) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->hubMap.homeWanderingMonSpecies) + \
    SERIALIZED_DATA_SIZE((saveBlock)->hubMap.weatherState) + \
    SERIALIZED_DATA_SIZE((saveBlock)->timeOfDayMinutes) + \
    SERIALIZED_DATA_SIZE((saveBlock)->seasonCounter) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->safariMons) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->daycarePokemon) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->difficultyConfig.toggleBits) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->difficultyConfig.rangeValues) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->dynamicUniquePokemon) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->safariMonCustomIds) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->monMasteryFlags) + \
    sizeof(bool8) + /* isDebug */ \
    ROGUE_SERIALIZED_DEBUG_CONFIG_SIZE + \
    sizeof(struct RogueRideMonState) + \
    SERIALIZED_ARRAY_SIZE((saveBlock)->adventureReplay) + \
    sizeof(gRngRogueValue) + \
    sizeof(struct RogueRunRestoreBlock) + \
    sizeof(struct RogueRunData) + \
    sizeof(gRogueAdvPath.currentRoomParams) + \
    sizeof(gRogueAdvPath.currentRoomType) \
)

STATIC_ASSERT(ROGUE_SAVE_SERIALIZED_SIZE(((struct RogueSaveBlock *)0)) < ROGUE_SAVE_BLOCK_CAPACITY, RogueSaveBlockSize);
